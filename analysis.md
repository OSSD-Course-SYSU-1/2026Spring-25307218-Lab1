...
│       │   │
│       │   ├── pages/
│       │   │   └── Index.ets                    #     主页面（双层Canvas + 底部工具栏 + 手势处理 + 作图工具面板）
│       │   │
│       │   ├── view/
│       │   │   └── myPaintSheet.ets             #     半模态设置面板（笔刷/颜色/RGB滑块/不透明度/粗细）
│       │   │
│       │   └── viewmodel/                       #     视图模型层（MVVM）
│       │       ├── DrawInvoker.ets              #       绘制命令管理器（命令模式：撤销/重做/执行）
│       │       ├── IBrush.ets                   #       笔刷接口IBrush + NormalBrush + FountainPenBrush
│       │       ├── IDraw.ets                    #       绘制接口IDraw + DrawPath + ShapeDraw
...
提供 8 种预设颜色（红、绿、蓝、橙、黄、紫、紫红、黑），点击选中时显示主题蓝色边框高亮。

### 5.2 RGB 三色滑块

在预设色板下方提供 R/G/B 三个滑块（0-255）：
- 红色滑块：标识色 `#E90808`
- 绿色滑块：标识色 `#63B959`
- 蓝色滑块：标识色 `#0A59F7`
...
`ShapeDraw extends DrawPath`，包含 `shapeType`、`startX/Y`、`endX/Y` 属性，`draw()` 方法根据 `shapeType` 绘制直线/圆/矩形。
...
- 点击"画笔"按钮弹出，包含：笔刷选择（3种）、颜色选择（预设+RGB滑块+当前预览）、不透明度、粗细
